...
app.get('/api/dashboard/overview', authMiddleware, async (req, res) => {
  try {
    ...
    const workflowsResult = await n8nPool.query('SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE active = true) as active FROM workflow_entity');
    ...
    const topFailedWorkflowsResult = await n8nPool.query(`
      SELECT 
        w.id as "workflowId",
        w.name as workflow_name,
...
app.get('/api/workflows', authMiddleware, async (req, res) => {
  try {
    let workflows = [];
    try {
      const result = await n8nPool.query('SELECT * FROM workflow_entity ORDER BY "updatedAt" DESC');
...
app.get('/api/executions', authMiddleware, async (req, res) => {
  try {
    ...
    try {
      ...
      const result = await n8nPool.query(query, params);
...
app.get('/api/dashboard/performance', authMiddleware, async (req, res) => {
  try {
    const performanceResult = await n8nPool.query(`
      SELECT 
        w.id,
        w.name,
        w.active,
...
app.get('/api/dashboard/failures/detailed', authMiddleware, async (req, res) => {
  try {
    ...
    const detailedFailuresResult = await n8nPool.query(`
      SELECT 
        e.id,
        e."workflowId",
        w.name as workflow_name,
...
app.get('/api/dashboard/insights', authMiddleware, async (req, res) => {
  try {
    ...
    const failureStatsResult = await n8nPool.query(`
      SELECT 
